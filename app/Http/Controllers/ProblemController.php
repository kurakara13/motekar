...
class ProblemController extends BaseController
{
    ...
    function problempoolInterest(Request $request, $id){
        ...
        $interest->motivate = $request->motivasi;
        $interest->idea = $request->idea;
        $interest->expected_role = $request->role;
        $interest->save();

        $problem = Problem::find($id);
        $user = User::where('id',$problem->user_id)->first();
        $event = Auth::user()->name.' telah join ke problem anda!!!';
        (new Notification)->problemInterest($event, $interest, $user);
